...
namespace Core.Web.Security
{
    public class CustomSecurityTokenService : SecurityTokenService
    {
        ...
        {
            ////http://www.wiktorzychla.com/2010/12/wif-ws-federation-and-single-sign-out.html
            // remember all the RPs to be able to perform a single sign-out
            //SingleSignOnManager.RegisterRP(scope.ReplyToAddress);
            
            //Jika login menggunakan Forms Authentication
            //var claims = new[]
            //    {
            ...
            var claims = new[]
                {
                    new Claim(System.IdentityModel.Claims.ClaimTypes.Name, principal.Identity.Name),
                    new Claim(System.IdentityModel.Claims.ClaimTypes.NameIdentifier, ((ClaimsIdentity)principal.Identity).Claims.FirstOrDefault(c=>c.Type == ClaimTypes.NameIdentifier).Value),
                    new Claim(ACSConstants.IdentityProviderClaimType, ((ClaimsIdentity)principal.Identity).Claims.FirstOrDefault(c=>c.Type == ACSConstants.IdentityProviderClaimType).Value)                    
                };
            var identity = new ClaimsIdentity(claims);
            return identity;
        }
